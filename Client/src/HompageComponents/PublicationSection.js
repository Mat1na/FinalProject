    ...
    <Container>
      <Row>
        ...
        <Col className="col-lg-8">
          <div><Link to={"/publicationlist"} className='allpublications'>Check out all publications</Link></div>
          {lastPublications.length > 0 && lastPublications.map((publication, index) => {
            return <div key={publication.order}><Link to={`/publication/${publication.journal.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase()}-${publication.order}`} className="stretched-link journallink">
